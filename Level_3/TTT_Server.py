...
pygame.display.set_caption ('TTT_Server')
col1 = (0,0,0)
col2 = (255,255,0)
...
while True:
    for event in pygame.event.get ():
        if event.type == QUIT:
            pygame.quit ()
            exit ()
        elif event.type == MOUSEBUTTONDOWN and event.button == 1 and pausev == False:
            x,y = event.pos
            conn.sendall ((str(x) + ',' + str (y)).encode ())
            conds (x,y)
            pausev = True
    if pausev == True:
        data = conn.recv (1024)
        data = data.decode ()
        recvx,recvy = data.split (',')
        recvx = int (recvx)
        recvy = int (recvy)
        conds (recvx,recvy)
        pausev = False
